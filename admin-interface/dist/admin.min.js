...
angular.module('yetienceApp', ['ui.router', 'formly', 'formlyBootstrap', 'jsonFormatter', 'ui.bootstrap', 'angular-md5', 'ngCookies', 'LocalStorageModule', 'angularTrix', 'ngTagsInput'])
    ...
    .config(['$stateProvider', '$urlRouterProvider', function ($stateProvider, $urlRouterProvider) {
        ...
        $stateProvider
            ...
            .state('list', {
                url: '/list',
                templateUrl: yetience.adminPath + '/src/partials/home.html',
                controller: 'ListController'
            })
            .state('premium', {
                url: '/premium',
            ...
            .state('build.apikey', {
                url: '/apikey',
                templateUrl: yetience.adminPath + '/src/partials/build.apikey.html',
                controller: 'buildAPIKeyController'
            })
            .state('extensions', {
                url: '/extensions',
...
angular.module('yetienceApp')
    .controller('ListController', ['$scope', 'SettingsService', '$state', '$rootScope', 'UtilsService', 'CommService', 'UpgradeService', '$location', 'localStorageService', '$modal', 'configurationFields', '$timeout', 'FeatureStatusService', 'WidgetUpdate', function ($scope, SettingsService, $state, $rootScope, UtilsService, CommService, UpgradeService, $location, localStorageService, $modal, configurationFields, $timeout, FeatureStatusService, WidgetUpdate) {
        ...
        $rootScope.$on('setup_initialized', function () {
            ...
            fetchSubscriberCount(SettingsService.setup())
        ...
        $scope.isEmailTheme = function (index) {
            ...
            if (WidgetUpdate.isEmailPopup(widgets[index])) {
                if (UtilsService.checkNested(widgets[index], ['components', 'emailSubscription', 'values', 'provider'])) {
                    ...
                    return true
                    /*
                    //Not showing View Email list if any other email provider is connected
        ...
        $scope.addonList = [1, 2, 3, 4, 5, 6, 7, 8]
        ...
        */
        ...
        $scope.addonGroups = {
            ...
            bundle: {
                title: 'Get all the Premium Features',
                monthly: 15,
                annual: 10
            },
            individual: {
                title: 'Get a Feature you need',
                monthly: 8,
                annual: 5
            },
        ...
        $scope.addonShow = function (section, feature) {
            if (section == 'bundle' && feature == 'premiumSubscription') {
                return true
            }
            if (section == 'individual' && feature != 'premiumSubscription') {
                return true
            }
        ...
        function fetchSubscriberCount(setup) {

            CommService.getSubscriberCount(setup.id)
                .then(function (count) {
                    setup.subscriberCount = count
                })
        }

        $scope.ext = {}
        $scope.activate = function (license, extension) {
            $scope.activationInProgress = true

            CommService.activate(license, extension, SettingsService.setup().id)
                .then(function (activation_response) {
                    console.log('activation response is ', activation_response)
                    $scope.activationInProgress = false
                    if (!activation_response.status.success) {
                        $scope.activationFailed = true
                        $scope.activationError = activation_response.status.error
                    } else {
...
angular.module('yetienceApp')
    .controller('MainController', ['$scope', '$rootScope', 'SettingsService', 'CommService', '$location', '$state', function($scope, $rootScope, SettingsService, CommService, $location, $state) {
        ...
        console.log("$scope.R.setupDateTime == ",$scope.R.setupDateTime);
        if($scope.R.setupDateTime)
        {
            console.log("IN IF---");
        }
        else
        {
            //if its already set, dont do anything. else set it
            console.log("SAVING SETUP TIME------");
            $scope.R.setupDateTime = new Date().toLocaleString();
            console.log($scope.R.setupDateTime);
        }
...
angular.module('yetienceApp')
    .controller('buildSelectController', ['$scope', 'SettingsService', '$state', '$rootScope', 'CommService', 'UtilsService', 'WidgetUpdate', '$modal', 'UpgradeService', function($scope, SettingsService, $state, $rootScope, CommService, UtilsService, WidgetUpdate, $modal, UpgradeService) {
        ...
        $scope.selectTheme = function(label, theme) {
            ...
            //for powered by yeloni message -> putting theme type on widget
            if(theme.categories.indexOf('share') > -1){
                $rootScope.widget.isSocial = true
            }

             if(theme.categories.indexOf('subscribe') > -1){
                $rootScope.widget.isEmail = true
            }
            ...
            controller: ['$rootScope', '$element', '$scope', 'SettingsService', 'CommService', 'UtilsService', function ($rootScope, $element, $scope, SettingsService, CommService, UtilsService) {
                ...
                $scope.$watch('provider', function (new_val, old_val) {
                    if (new_val) {
                        ...
                        CommService.createNewEvent("provider_selected", {
                            'provider': new_val
                        })
                ...
                function fetchAndAssignLists(provider) {
                    ...
                    if (provider && provider.indexOf('Yeloni') < 0 && provider.indexOf('Sendy') < 0) {
                        ...
                        if ($scope.emailConfig && $scope.emailConfig.lists) {
                            $scope.emailFields[1].templateOptions.options = $scope.emailConfig.lists
                        }
            ...
            controller: ['$scope', '$attrs', '$rootScope', '$element', 'UtilsService', '$timeout', function($scope, $attrs, $rootScope, $element, UtilsService, $timeout) {
                ...
                if ($attrs.emailPart == 'name') {
                    $rootScope.$watch('widget.components.gdprPlaceholder.values', function(new_val) {
                        ...
                        console.log('GDPR values changed')
                    }, true)
                }

                if ($attrs.emailPart == 'gdpr') {
                    console.log('Email part is gdpr')
                    console.log('Components ', $rootScope.widget.components)
...
angular.module('yetienceApp')
    .service('CommService', ['$http', '$q', '$rootScope', 'md5', function ($http, $q, $rootScope, md5) {
        ...
        this.isReferrerPresent = function () {
            var D = $q.defer()
        ...
        this.getEmails = function (website_id, list, page) {
            ...
            var limit = 10
            return getAndResolve('/api/EmailSubscriptions?filter[where][website_id][like]=' + website_id + '&[where][list]=' + list + '&filter[limit]=' + limit + '&filter[skip]=' + current_page + '&filter[order]=timestamp%20DESC')
        }
        ...
        this.getEmailLists = function (provider, website_id) {
            ...
            setTimeout(function () {
                getAndResolve('/api/EmailSubscriptions/lists?provider=' + provider + '&website_id=' + website_id)
        ...
        this.activate = function (license, extension, website_id) {
            return postAndResolve('/subscription/api/activations/activate', {
                license: license,
        ...
        this.getSubscriberCount = function (website_id) {
            return getAndResolve('/api/EmailSubscriptions/count?where[website_id][like]=' + website_id)
        }
    ...
    .service('WidgetUpdate', ['CommService', '$state', '$timeout', 'SettingsService', 'UtilsService', '$rootScope', '$modal', '$q', function (CommService, $state, $timeout, SettingsService, UtilsService, $rootScope, $modal, $q) {
        ...
        */
        var states = {
            ...
            design: {
                ...
                title: function (widget) {
                    return (firstWidget()) ? 'Generate API Key' : 'Save my Design'
            ...
            apikey: {
                show: false,
                tabTitle: 'Generate API Key',
                title: 'Save my Design',
                create: function () {
                    //show this only if no widget has been created yet
                    return SettingsService.setup().widgets.length == 0
                },
                edit: false,
        ...
        function isEmailPopup(widget) {
            if (!$rootScope.themes) {
                return false //themes are not yet fetched
            }
            if (widget) {
                return ($rootScope.themes[widget.theme].categories.indexOf('subscribe') >= 0)
            } else {
        ...
        function checkAndAskWidgetName(widget) {
            ...
            if (angular.isString(widget_name)) {
                ...
                var modalInstance = $modal.open({
                    controller: ['$rootScope', 'WidgetUpdate', function (R, WU) {
                        this.R = $rootScope
                        this.WU = WU

                        this.poweredBy = function () {
                            return !R.SETUP.poweredBy || R.SETUP.customPoweredBy
                        }
        ...
        this.changePoweredBy = function (fromCreate) {
            //Create a new variable customPoweredBy so that we can keep the 'powered by yeloni' option always available to downloaded plugin users
            $rootScope.SETUP.customPoweredBy = true
            //return if the widget is being created
            if (fromCreate == true) {
                return
            }
            var message = "'Powered by Yeloni' message will now be displayed",
                button = 'Enable'
            // console.log($rootScope.SETUP.poweredBy) 
            if ($rootScope.SETUP.poweredBy == false) {
                message = "'Powered by Yeloni' message will no longer be displayed", button = 'Disable'
            }

            SettingsService.saveSetup('<center> ' + message + ' </center>', button)
        }
    }])
angular.module('yetienceApp')
...
angular.module('yetienceApp')
    .constant('configurationFields', {
        ...
        when: [{
            ...
            type: "textarea",
            key: 'when.customLink.large',
            hideExpression: "(model.when.link.large != 'custom' || model.when.large != 'link')",
            templateOptions: {
                label: "Enter the url stubs",
                description: 'Enter multiple values separated by a comma',
                rows: 2,
                cols: 15
            }
        }, {
            ...
            type: "textarea",
            key: 'when.customLink.small',
            hideExpression: "(model.when.link.small != 'custom' || model.when.large != 'link')",
            templateOptions: {
                label: "Enter the url stubs separated by a comma",
                description: 'Enter multiple values separated by a comma',
                rows: 2,
                cols: 15
            }
        ...
        where: [{
            ...
            key: "where.pageTypes.custom",
            type: "input",
            hideExpression: "(model.where.showOrHide != 'show' || model.where.specific.selector != 'custom')",
            templateOptions: {
                label: "Enter the custom page type"
            }
        }, {
            type: "radio",
            ...
            key: "where.pageTypes.custom",
            type: "input",
            hideExpression: "(model.where.showOrHide != 'hide' || model.where.hideOn.hideselector != 'custom')",
            templateOptions: {
                label: "Enter the custom page type"
            }
        }],
