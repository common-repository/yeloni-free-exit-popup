...
class Yetience
{
	...
	function add_admin_page(){
 	...
 	function add_client_page(){
 		...
 		if(isset($setup->widgets) && count($setup->widgets) > 0){
 			/*
 	...
 	function register_js($js_list){
 		foreach ($js_list as $js_file) {
			 if(strpos($js_file,'https://') !== false){
				wp_register_script($js_file, $js_file);
			 }else{
	...
	function decode_for_client(){
		/*
			1. Instead of base64 encoding whole setup, just to html encoding the setup variable
			2. Retain only components.values . Remove other attributes from components
			3. Remove raw attribute
			4. base64 decode of styles
			5. base64 decode of rendered
		*/
		$stored_setup = $this->get_decoded_setup();
		$widgets = $stored_setup->widgets;

		foreach($widgets as $widget){
			$widget->raw = "";
			$widget->rendered = base64_decode($widget->rendered);
			$widget->styles = base64_decode($widget->styles);
		}


		return urlencode(json_encode($stored_setup));
	}
 	...
 	//STATIC FUNCTIONS BELOW
 		...
 		//echo "post type count- ".$post_type_count;
 			...
 			for($i = 0;$i<$post_type_count;$i++){
 				$pages_of_this_type = get_posts(array('posts_per_page' => 100, 'offset' => 0, 'post_type' => $post_types[$i]));
